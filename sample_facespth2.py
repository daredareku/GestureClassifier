...
gesture_model = GestureClassifier(num_classes)
gesture_model.load_state_dict(torch.load('gesture_recognition_model.pth'))
gesture_model.eval()
...
try:
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            with torch.no_grad():
                gesture_output = gesture_model(face_image.unsqueeze(0))
                gesture_prediction = torch.argmax(gesture_output, dim=1).item()
